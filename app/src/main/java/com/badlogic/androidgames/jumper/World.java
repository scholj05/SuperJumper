...
public class World {
	...
	private void generateLevel() {
		...
		{
			float y = WORLD_WIDTH / 2;
			float maxJumpHeight = Bob.BOB_JUMP_VELOCITY * Bob.BOB_JUMP_VELOCITY / (2 * gravity.y);
			while (y < WORLD_HEIGHT - Platform.PLATFORM_HEIGHT / 2) {
				count++;
				int type = rand.nextFloat() > 0.8f ? Platform.PLATFORM_TYPE_MOVING : Platform.PLATFORM_TYPE_STATIC;
				float x = rand.nextFloat() * (WORLD_WIDTH - Platform.PLATFORM_WIDTH) + Platform.PLATFORM_WIDTH / 2;
	...
	private void updateBob(float deltaTime, float accelX) {
		...
		if (Settings.gameDirectionUp) heightSoFar = Math.max(bob.position.y, heightSoFar);
		else heightSoFar = Math.min(bob.position.y, heightSoFar);
	}
